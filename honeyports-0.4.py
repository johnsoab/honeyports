...
# Updated 20160316 Andy Johnson -- Added Windows Event Warning and made python3 compatible
...
# This makes this far less OS agnostic, just deal with it.
...
print("I will listen on TCP port number: %s" % port) 

# Determine which platform we are running on

platform = platform.system()
print("Honeyports detected you are running on: %s" % platform)
...
while True:
   ...
   if platform == "Windows":
       ...
       win32evtlogutil.ReportEvent("HoneyPortAlert", 4242, eventType = win32evtlog.EVENTLOG_WARNING_TYPE, strings = [ "Detected HoneyPort Hit", "HoneyPort hit from " + hostname + " on port " + str(port) ], data = (hostname + "\0" + str(port)).encode("ascii"));
   ...
   if fw_result:
     ...
     print("I just blocked: %s" % hostname)
...
# Flush all firewall rules, quit or print the rules
   ...
   mydata = input('Enter Commands: q=quit f=flush rules p=print rules.')

   if mydata == "f":
    print("Flushing firewall rules...")
    print("Flush command is: %s" % str(flush))
